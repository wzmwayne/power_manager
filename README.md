...
- 物理级熔断自救（多目录冗余）：支持 8 个目录的 `pmoff` 文件探测 + `/pmon` 授权信标机制，无需卸载模块即可彻底断电。
...
模块启动（注入前）按下表顺序轮询，任一 `pmoff` 存在即静默退出；`/pmon` 不存在或不可读同样熔断（未授权）。
...
首次授权：App 内点击「允许模块运行」→ 3 秒倒计时确认 → 创建 `/pmon` + 删除所有 `pmoff` → 重启生效。
